...
select c.storestate as State, sum(s.quantity) as TotalQuantity from stores c 
join sales s on s.customerID = s.customerID
group by c.storestate
GO
...
select distinct c.CustomerID, c.FirstName, c.LastName, count(distinct st.storeState) as StateCount from customers c 
join sales s on s.customerID = c.CustomerID 
join stores st on st.StoreID = s.StoreID
group by c.CustomerID, c.FirstName, c.LastName

-- ===================================================== -- Task 13: Analyze sales trends by month using a temporary table.
create table #MonthlySales (MonthlySalesID int IDENTITY(1,1) PRIMARY key, SaleYear int, SaleMonth int, TotalSales Decimal(12,2))

insert into #MonthlySales
select YEAR(saleDate) as SaleYear, MONTH(saleDate) as SaleMonth, sum(TotalAmount) as TotalSales from sales group by YEAR(saleDate), MONTH(saleDate)

IF OBJECT_ID('tempdb..#MonthlySales') IS NOT NULL
    select top 1 SaleYear, SaleMonth, TotalSales from #MonthlySales order by TotalSales DESC
GO
