...
class InsuranceTransformer:
    ...
    def _calculate_customer_age(self, birth_date: str) -> int:
        ...
        try:
            start = datetime.strptime(birth_date, "%Y-%m-%d").date()
            today = date.today()
            age = (today.year - start.year)
    ...
    def _classify_customer_risk(self, risk_score: float, age: int) -> str:
        ...
        # TODO: Your implementation here
        # Remove this pass statement and implement the method
        if age < 25 or risk_score >= 3.0:
            return "High"
        elif risk_score < 2.0 and age >= 25:
            return "Low"
        else:
            return "Medium"
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_ages = [
            ...
            ("1990-12-25", "Should be around 35"),
            ("", "Should be 0"),
            ("invalid", "Should be 0"),
        ]
        ...
        print("\nTesting TODO #3: _standardize_phone")
        test_cases = [
        ...
        for phone, expected in test_cases:
            result = transformer._standardize_phone(phone)
            print(f"Input: {phone}\nExpected: {expected}\nGot: {result}\n")

        print("\nTesting TODO #4: _determine_premium_tier")
        test_tier = [(999, "Economy"), (1000, "Standard"), (1999, "Standard"), (2000, "Premium")]

        for premium, expected in test_tier:
            result = transformer._determine_premium_tier(premium)
            print(f"Input: {premium}\nExpected: {expected}\nGot: {result}\n")

        print("\nTesting TODO #5: _validate_claim_amount")
        test_claims = [
            (0, 100, False),
            (1, 100, True),
            (100, 100, True),
        ...
        for claim_amount, coverage_amount, expected in test_claims:
            result = transformer._validate_claim_amount(claim_amount, coverage_amount)
            print(
                f"Input: claim_amount={claim_amount}, coverage_amount={coverage_amount} → "
