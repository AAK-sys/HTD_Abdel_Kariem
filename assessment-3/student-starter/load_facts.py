...
class InsuranceFactLoader:
    ...
    def _load_single_claim_fact(self, resolved_claim: Dict[str, Any]) -> bool:
        ...
        sql = """
            INSERT INTO fact_claims
            (claim_id, customer_key, policy_key, agent_key,
            filed_date_key, closed_date_key,
            claim_amount, coverage_amount, deductible_amount,
            payout_amount, processing_days, claim_status,
            created_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, GETDATE())
            """
        ...
        params = (
            ...
            resolved_claim.get("closed_date_key"),
        ...
        try:
            ...
            self.logger.warning(f"Failed to insert claim {resolved_claim.get('claim_id')}: {e}")
            return False
