...
class InsuranceDimensionLoader:
    ...
    def _process_customer_batch(
        ...
        """Process a batch of customer records for dimension loading"""
        ...
        for customer in customers:
            try:
                ...
                if existing:
                    # Update existing customer (SCD Type 1)
                    update_sql = """
                    UPDATE dim_customer
                    SET customer_id = ?,
                        first_name = ?,
    ...
    def _process_policy_batch(self, policies: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        for policy in policies:
            try:
                ...
                if existing:
                    # Update existing policy (SCD Type 1)
                    update_sql = """
                    UPDATE dim_policy
                    SET policy_id = ?,
                        policy_type = ?,
